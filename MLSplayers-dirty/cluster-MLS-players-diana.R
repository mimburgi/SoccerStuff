...
trimmed<-dat[,c('percA3pass', 'percM3pass', 'percD3pass', 'percChain', 'ShotChainPerc', 'KPChainPerc', 
                'xG', 'xA', 'xGChain', 'xB')]
rownames(trimmed)<-dat$Player
scaled<-scale(trimmed) %>% as.data.frame()
scaled[scaled < -3]<- -3


library(cluster)

res.diana<-diana(scaled, metric = 'manhattan')
hcd<-as.dendrogram(res.diana)
fviz_dend(res.diana, k=8, color_labels_by_k = T, show_labels = F, rect = T)


hcut8=cutree(res.diana, 8)

library(dendextend)
colorCodes <- c("red","green","blue","cyan", 'black', 'purple', 'yellow', 'darkgreen', 'gray', 'deeppink', 'darkslateblue'); 
labels_colors(hcd) <- colorCodes[hcut8][order.dendrogram(hcd)]
plot(hcd)
plot(res.diana, which.plots = 2, cex=.01)
library(factoextra)

plotclusters(scaled, hcut8)

labs<-as.factor(hcut8)
levels(labs)<-c('Effective MF Progressor', 'MF Creator', 'Defender', 'Final Third Creator', 'Ineffective MF Progressor', 'Support Attacker', 'Poacher', 'Link-up Striker')
dat$lab<-labs
levels(dat$lab)<-c('Effective MF Progressor', 'MF Creator', 'Defender', 'Final Third Creator', 'Ineffective MF Progressor', 'Support Attacker', 'Poacher', 'Link-up Striker')
