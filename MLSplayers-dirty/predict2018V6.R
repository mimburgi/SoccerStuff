library(caret)
library(dplyr)
current<-read.table('labeled5.txt', header = T) #2019 data
...
usedvars<-c('shots', 'KP', 'xBperc')
currentX<-select(current, one_of(usedvars))

currentY<-current$hybridrole

set.seed(100)
trainIndex<-createDataPartition(currentY, p=.8, list = F)
...
gbm<-train(currentX[trainIndex,], currentY[trainIndex], method = 'gbm',
          trControl = trainControl(method = 'repeatedcv', savePredictions = T),
          tuneGrid = expand.grid(interaction.depth=c(1,2,3,4,6),
...
confusionMatrix(predict(rf, currentX[-trainIndex,]), currentY[-trainIndex])
...
testprevX<-select(testcurr, one_of(usedvars))

testlabs<-predict(rf, testprevX)

sum(testlabs==testcurr$hybridrole)/length(commonplayers)

confusionMatrix(testlabs, testcurr$hybridrole)
