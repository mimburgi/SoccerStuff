...
usedvars<-c("shots", "npxG", "KP", 
            'xA', "xGChain", 
            "xB", "PassPct", 'passprg', 
            'Crs','penCrs', 'SuccDrib', 
            'targeted', 'shortpasspct', 'longpasspct') 
...
set.seed(20)
tsne<-Rtsne::Rtsne(bothscaled, perplexity=30)$Y #17
plot(tsne)
...
compclusts<-as.factor(clara$clustering) #70
# compclusts<-as.factor(pam$cluster)
compclusts<-as.factor(hk$cluster) #70
# compclusts<-as.factor(hk2$cluster)
# compclusts<-as.factor(hc)
# compclusts<-as.factor(hc2)

## name cluster levels ####
levels(compclusts)<-c('Playmaker','Outlet',
                      'Shuttler','Wide Attacker',
                      'Direct Progressor','Central',
                      'Crossing Specialist')
