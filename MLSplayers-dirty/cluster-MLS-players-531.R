...
pca<-PCA(scaled)
fviz_screeplot(pca)
pcadecomp<-preProcess(allscaled, method='pca', pcaComp = 3) %>% predict(allscaled)



kmm<-kmeans(pcadecomp, centers = 3)
library(NbClust)
kmeansnb<-NbClust(scaled, max.nc = 7, method = c('kmeans'))
fviz_nbclust(kmeansnb)

hcnb<-NbClust(scaled, max.nc = 7, method = c('complete'))
fviz_nbclust(hcnb) #5 maybe

hcres<-hclust(dist(scaled))
cutfive<-cutree(hcres, 5)
fviz_dend(hcres, k=5, show_labels = F)

plotclusters(scaled, cutfive)

clusters<-as.factor(cutfive)
levels(clusters)<-c('MF Creator', 'MF Recycler', 'Creative Attacker', 'Defensive Outlet', 'Finisher')

dat$role<-clusters
### separate roles ####

mfc<-dat[dat$role=='MF Creator',] 
mfr<-dat[dat$role=='MF Recycler',] 
catt<-dat[dat$role=='creative Attacker',] 
def<-dat[dat$role=='Defensive Outlet',] 
fin<-dat[dat$role=='Finisher',] 

fins<-which(dat$role=='Finisher')
defs<-which(dat$role=='Defensive Outlet')
catts<-which(dat$role=='Creative Attacker')
cmf<-which(dat$role=='MF Creator')
mfr<-which(dat$rol=='MF Recycler')

## finishers ####
select_if(fin, is.numeric) %>% apply(2, rsd)
candidate_finvars<-c('xG', 'xA', 'shots', 'KP', 'percChain', 'KPChainPerc',
                     'ShotChainPerc', 'percChain')
finscaled<-select(fin, one_of(candidate_finvars)) %>% scale() %>% as.data.frame()
pcafin<-PCA(finscaled)
fviz_screeplot(pcafin) #clear level off at three
for (i in 1:3){
...
finhcnb<-NbClust(findecomp, max.nc = 4, method = c('kmeans'))
...
hcresfin<-hclust(dist(finscaled))
fincut<-cutree(hcresfin, 3)
fviz_dend(hcresfin, k=3)
