...
set.seed(250)
tsne<-Rtsne::Rtsne(bothscaled, perplexity=27)$Y
plot(tsne)
...
# compclusts<-as.factor(km5$cluster) #79.9
...
# compclusts<-as.factor(km7$cluster) #75.4
# compclusts<-as.factor(clara7$cluster) #73.7
# compclusts<-as.factor(pam7$cluster) #73.1
compclusts<-as.factor(hk7$cluster) #75.4
# compclusts<-as.factor(hk72$cluster)#75.4
...
## name cluster levels ####
levels(compclusts)<-c('DLP', 'AM Creator', 'Hybrid Attacker', 
                      'Defender', 'Attacking Shuttler', 'Finisher',
                      'Shuttler')
