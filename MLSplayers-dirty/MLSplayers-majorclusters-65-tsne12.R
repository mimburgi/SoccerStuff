...
usedvars<-c("shots", "KP", "xG", 
            "percChain", "xGChain", 'ShotChainPerc', "KPChainPerc", 
            "xBperc", "xPassPerc", "PassPct", 'Crs','Vertical', 
            'SuccDrib') 
...
#huge increase
set.seed(25)
tsne<-Rtsne::Rtsne(bothscaled, perplexity=perp)$Y #23
plot(tsne)
...
ggplot(hopkinsdftsne, aes(x=clusters, y=stat)) + geom_point() + geom_line() +
  ggtitle(paste('hopkins statistic, tsne perplexity', as.character(perp)))
...
compclusts<-as.factor(km$cluster) #70.8
...
compclusts<-as.factor(hc2) #71.4, but this one yields a small group that has no connections across years
...
plotclusters(bothscaled[,c(10:13)], compclusts)
