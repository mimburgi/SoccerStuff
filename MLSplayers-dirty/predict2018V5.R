library(caret)
library(dplyr)
current<-read.table('labeled4.txt', header = T) #2019 data
...
for (player in allplayers){
  ...
  if(player %in% totalpass$Player){
    ...
    prev$PassScore[prev$Player==player]<-totalpass$Score.96[totalpass$Player==player]
  }
}
...
prev$A3perc<-prev$A3Passes/prev$Passes
prev$M3perc<-prev$M3Passes/prev$Passes
prev$D3perc<-prev$D3Passes/prev$Passes

## start by predicting on 2019 ####
usedvars<-c('shots', 'KP', 'xB', 'xBperc', 'PassScore',
            'percChain', 'ShotChainPerc', 'KPChainPerc', 
            'xGChain','PassScore', 'PassPct', 'A3perc', 'M3perc', 'D3perc')
currentX<-select(current, one_of(usedvars))

currentY<-current$subtype

set.seed(10)
trainIndex<-createDataPartition(currentY, p=.5, list = F)

set.seed(0)
rf<-train(currentX[trainIndex,], currentY[trainIndex], method = 'rf',
          trControl = trainControl(method = 'repeatedcv', savePredictions = T),
          tuneLength = 10)
set.seed(0)
gbm<-train(currentX[trainIndex,], currentY[trainIndex], method = 'gbm',
          trControl = trainControl(method = 'repeatedcv', savePredictions = T),
          tuneGrid = expand.grid(interaction.depth=c(1,2,3),
                                 n.trees=seq(50, 350, 50),
                                 shrinkage=c(.01, .1),
                                 n.minobsinnode=10))
gbmclass1<-predict(gbm, currentX[-trainIndex,])
rfclass1<-predict(rf,currentX[-trainIndex,])



sum(gbmclass1==currentY[-trainIndex])/length(currentY[-trainIndex])
sum(rfclass1==currentY[-trainIndex])/length(currentY[-trainIndex])

gbmprobs1<-predict(gbm, currentX[-trainIndex,], type = 'prob')
rfprobs1<-predict(rf, currentX[-trainIndex,], type = 'prob')

gbmprobs1$maxprob<-apply(gbmprobs1, 1, max)
rfprobs1$maxprob<-apply(rfprobs1, 1, max)

gbmprobs1$uncertain<-ifelse(gbmprobs1$maxprob < .7, T, F)
rfprobs1$maxprob<-ifelse(rfprobs1$maxprob < .7, T, F)

misclasses<-which(gbmclass1!=currentY[-trainIndex])
gbmprobs1$misclass<-F
gbmprobs1$misclass[misclasses]<-T

confusionMatrix(gbmprobs1$uncertain, gbmprobs1$misclass)

sum(gbmprobs1$uncertain==T[misclasses])/length(gbmprobs1$uncertain==T)

confusionMatrix(gbmpreds1, currentY[-trainIndex])


## look at prediction probs ####
head(predict(rf, type = 'prob'))

## match players in each dataframe ####

commonplayers<-intersect(current$Player, prev$Player)
testprevs<-subset(prev, Player %in% commonplayers) %>% arrange(Player)

testcurr<-subset(current, Player %in% commonplayers) %>% arrange(Player)

prevX<-select(prev, one_of(usedvars))

testlabs<-predict(gbm, select(testprevs, one_of(usedvars)))

sum(testlabs==testcurr$subtype)/length(commonplayers)

confusionMatrix(testlabs, testcurr$subtype)


### predict on 2018 ####
set.seed(20)
predgbm<-predict(gbm, newdata = prevX, type='prob')
head(predgbm)
predgbm$maxprob<-apply(predgbm, 1, max)
predgbm$uncertain<-ifelse(predgbm$maxprob < .7, T, F)

classgbm<-predict(gbm, newdata = prevX)




library(dplyr)
prevwithpreds<-prev %>% mutate(predrole=classgbm) %>% mutate(uncertain=predgbm$uncertain)
