...
usedvars<-c("shots", "npxG", 'ShotChainPerc', "KP", 
            'xA','TB',  "KPChainPerc", "xGChain", 
            "xBperc", "xPassPerc", "PassPct", 'passprg', "A3teamperc", 
            "M3teamperc","D3teamperc", 'Crs','penCrs','Vertical', 
...
set.seed(20)
tsne<-Rtsne::Rtsne(bothscaled, perplexity=21)$Y #21
plot(tsne)
...
compclusts<-as.factor(clara$clustering) #78 (10)
compclusts<-as.factor(pam$cluster) 
compclusts<-as.factor(hk$cluster) 
compclusts<-as.factor(hk2$cluster) #76
compclusts<-as.factor(hc) 
compclusts<-as.factor(hc2) #74

## name cluster levels ####
levels(compclusts)<-c('MF Orchestrator','Attacking FB',
                      3,4,
                      'Support FB','',
                      7,'MF Outlet',
                      9,'',
                      'Scoring Winger/Complete attacker','Finisher')
