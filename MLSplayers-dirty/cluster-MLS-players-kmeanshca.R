...
agghc=function(data, k, method='ward.D2')
{
  res.hca<-dist(data, method = "euclidean") %>%
    hclust(method = method )
  plot(fviz_dend(res.hca, cex = 0.5, k = k, color_labels_by_k = TRUE))
  print(cutree(res.hca, k))
  print(aggregate(data, by=list(cutree(res.hca, k) %>% unname), mean))
}
...
### separate major playstyles ####
lowMF<-scaled[scaled$clustername=='low contributing MF',] %>% select_if(is.numeric) 
creator<-scaled[scaled$clustername=='primary creator',] %>% select_if(is.numeric)
lowD<-scaled[scaled$clustername=='low contributing defender',] %>% select_if(is.numeric)
DLP<-scaled[scaled$clustername=='DLP',] %>% select_if(is.numeric)
passD<-scaled[scaled$clustername=='skilled passing defender',] %>% select_if(is.numeric)
support<-scaled[scaled$clustername=='support attacker',] %>% select_if(is.numeric)
finisher<-scaled[scaled$clustername=='finisher',] %>% select_if(is.numeric)

## finisher hca ####
rownames(finisher)<-scaled$Player[scaled$clustername=='finisher']
finisher<-finisher[,c("shots", "KP", "xG", "xA", "A3Passes",
                      "NumChains", "xGChain", 'xB',
                      'TouchPerc')]
agghc(finisher, 8, method='complete')

select(finisher, -cluster)%>%
  FactoMineR::PCA() %>% fviz_contrib(choice = "var", axes = 3, top = 10)


## support attacker hca ####
rownames(support)<-scaled$Player[scaled$clustername=='support attacker']
support<-support[,c("shots", "KP", "xG", "xA", "A3Passes", "M3Passes",
                      "NumChains", "xGChain", 'xB',
                      'TouchPerc')]


agghc(support, 5, method='complete')

FactoMineR::PCA(support) %>% fviz_contrib(choice = "var", axes = 3, top = 10)
FactoMineR::PCA(support) %>% fviz_eig()
library(caret)
support_pca<-preProcess(support, method = 'pca', pcaComp=3) %>% predict(support)
