...
pcascaled<-caret::preProcess(bothscaled, method='pca', pcaComp=3) %>% predict(bothscaled)
...
set.seed(25)
tsne<-Rtsne::Rtsne(bothscaled, perplexity=6)$Y 
plot(tsne)
...
comp<-clValid::clValid(as.data.frame(tsne), nClust=5:18, method = 'complete',
                       clMethods = c('hierarchical', 'som',
                                     'diana', 'clara', 'pam'),
...
compclusts<-as.factor(km$cluster) #84
compclusts<-as.factor(clara$cluster) #
compclusts<-as.factor(pam$cluster) #
compclusts<-as.factor(hk$cluster) #73.6
...
plotclusters(bothscaled[,c(9:12)], compclusts)
plotclusters(bothscaled[,c(13:18)], compclusts)
plotclusters(bothscaled[,c(19:21)], compclusts)
...
## name cluster levels ####
levels(compclusts)<-c('MF Distributor 1', 'AM Creator', 'Attacking Shuttler',
                      'Hybrid Attacker','MF Shuttler', 'Safe Passing Def.',
                      'MF Distributor 2', 'Risky Passing Def.', 'Finisher',
                      'Pure Scorer')
