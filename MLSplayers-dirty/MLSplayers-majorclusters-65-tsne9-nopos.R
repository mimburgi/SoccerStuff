...
# compclusts<-as.factor(clara$clustering) #74 
# compclusts<-as.factor(pam$cluster)
# compclusts<-as.factor(hk$cluster) #77
# compclusts<-as.factor(hk2$cluster) #78
compclusts<-as.factor(hc) #78.5
# compclusts<-as.factor(hc2) 

## name cluster levels ####
levels(compclusts)<-c('MF Progressor','MF Support',
                      'B2B & Cross','Playmaker',
                      'Support Attacker','Defender',
                      'Finisher', 'MF Support')
...
plotclusters(bothscaled[,c(19)], compclusts)
