getWhoScoredStats = function(url,id){
  ...
  remDr$navigate("https://www.whoscored.com/Teams/26/")
  webElem1 <- remDr$findElement(value = '//a[@href = "#top-team-stats-detailed"]')
  webElem1$clickElement()
  webElem2 <- remDr$findElement("id", "subcategory")
  webElem2$clickElement()
  webElem2$sendKeysToElement(list(key="down_arrow"))
  ...
  #                                 key="enter"))
  webElem5 <- remDr$findElement(using = "id", value = "statistics-team-table-detailed")
  webElem5txt <- webElem5$getElementAttribute("outerHTML")[[1]]
  data <- XML::readHTMLTable(webElem5txt, header=TRUE, as.data.frame=TRUE)[[1]]
  # data <- xml2::read_html(remDr$getPageSource()[[1]]) %>%
  #   rvest::html_nodes("stage-goals-grid") %>%
  #   rvest::html_table(header=TRUE, as.data.frame=TRUE)
  # data = data[[1]]
