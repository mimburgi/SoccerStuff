...
attscaled<-players %>% select(all_of(attvars)) %>% 
...
set.seed(500)
dtsne<-Rtsne::Rtsne(attscaled, perplexity=10)
plot(dtsne$Y)
restsne<-dtsne$Y %>% as.data.frame()

library(cluster)
fanny4<-fanny(restsne, 4)
plot_cluster(restsne, fanny4$clustering)

resdb<-dbscan::dbscan(restsne, minPts=30, eps=4.2)
plot_cluster(restsne, as.factor(resdb$cluster))

reshdb<-dbscan::hdbscan(restsne, minPts = 10)
plot_cluster(restsne, reshdb$cluster)
plot_radars(reshdb$cluster, attscaled)

attplayers$Player[reshdb$cluster==1]

attclusts<-clar2$clustering %>% as.factor() %>%
  recode("0" = "Mobile Finisher", 
         "1" = 'Poacher', 
         "2" = 'Creator',
         "3" = "Mobile Finisher")
