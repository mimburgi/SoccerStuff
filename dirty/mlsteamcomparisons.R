...
write.table(raw_extra, 'newextra.txt', na = "", row.names = F, sep = "\t")
...
### try pca to isolate the long/cross vs short/TB axis ####
res.pca<-PCA(trim)
#second is the axis I care about, first is more related to amount of KP
fviz_screeplot(res.pca)
#two PCS matter a lot, drops off after

#simplify interpretation by plotting second PC against total KP

decomp<-predict(res.pca, trim)
#trim$totalKP_scaled<-scale(raw_extra$TotalKeyPasses)
trim$totalKP<-raw_extra$TotalKeyPasses
trim$PC2<-as.data.frame(decomp$coord)[,2]
trim$P1<-as.data.frame(decomp$coord)[,1]

plot(trim$P1, trim$PC2)

#export for viz in tableau
raw_extra$KPtypeaxis<-trim$PC2

write.csv(raw_extra, 'top4extra.csv', row.names = F)

write.table(raw_extra, 'top4extra.txt', row.names = F)

###
forkmeans<-select(raw_extra,c(TotalDribbles, LongKeyPasses, ShortKeyPasses,
                              CrossKP, ThroughballKP, percCounterShots, TotalKeyPasses,
                              League))


#find variables with high relative SD
RSD <- function(vector){
  mean=mean(vector)
  sd<-sd(vector)
  (sd/mean)*100
...
#remove variables related to goals, assists and saves. they aren't informative neough
candidates<-raw_extra %>% 
  select(-contains('Goal')) %>%
  select(-contains('Ass', ignore.case = F)) %>%
  select(-contains('Save'))
relSDs<-apply(select(candidates, -c(League,Team)), 2, RSD)

View(relSDs)

offcandidates<-cbind(
  select(candidates, contains('shot')),
  select(candidates, contains('KP')),
  select(candidates, League))
rSDsMLS<-apply(subset(offcandidates, League=="MLS")%>%select(-c(League)), 2, RSD)

knnset<-select(candidates, c(Team, League, LongKeyPasses,
                             ThroughballKP, CrossKP,
                             percCounterShots,percOutofBoxShots, 
                             OpenPlayShots, SetPieceShots))
knnsetscaled<-scale(select_if(knnset, is.numeric))
mlsteams<-which(knnset$League=="MLS")
euroteams<-which(knnset$League!="MLS")

library(FNN)
res.knn<-knn(train = knnsetscaled[euroteams,],
             test = knnsetscaled[mlsteams,],
             cl = knnset$Team[euroteams],
             k=1
)


mlscomp<-subset(knnset, League=='MLS') %>%
  mutate(eurocomp=as.data.frame(res.knn)[,1]) %>%
...
# 
# 
# euroteams<-kmeans(subset(forkmeans, League!='MLS') %>%select(-League),
#                   centers = nrow(subset(forkmeans, League!='MLS'))-1,
#                   nstart = 30)
# 
# mlspredict<-predict(euroteams, subset(forkmeans, League=='MLS') %>%select(-League))
# 
# length(rownames(mlspredict)%>%{unname(euroteams$cluster)==.})

mlsdf<-subset(forkmeans, League=='MLS')%>% 
  mutate(comparison=mlscomps) %>%
  mutate(team=subset(raw, League=='MLS')[['Team']])
